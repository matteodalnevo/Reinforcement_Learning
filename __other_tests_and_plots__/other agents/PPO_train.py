...
def learn(a2c_net, optimizer_actor, optimizer_critic, gamma, states, next_states, actions, rewards, gathered_old_probs, clip_ratio=0.1, entropy_coeff=0.01):
    ...
    with tf.GradientTape(persistent=True) as tape_actor, tf.GradientTape(persistent=True) as tape_critic:
        ...
        # Compute the actions probs for new policy
        action_probs = a2c_net.actor(states)
        # Compute the discounted rewards
        td_target = rewards + gamma * next_val
        # Compute advantages
        ...
        ratio = gathered_probs / gathered_old_probs
        
        # Compute the clipped probability ratio
        clipped_ratio = tf.clip_by_value(ratio, 1 - clip_ratio, 1 + clip_ratio)
        
        # Compute the surrogate objective
        surrogate = tf.minimum(ratio * tf.squeeze(advantages), clipped_ratio * tf.squeeze(advantages))
        ...
        # Optionally, compute the entropy bonus
        entropy = -tf.reduce_sum(action_probs * tf.math.log(action_probs), axis=1)
        entropy_bonus = tf.reduce_mean(entropy)

        # Combine actor loss and entropy bonus
        actor_loss -= entropy_coeff * entropy_bonus
...
def train(envs, a2c_net, optimizer_actor, optimizer_critic, gamma, max_steps, TEST):
  ...
  for step in trange(1, max_steps + 1):
      ...
      # Compute the length of the bodies and change rewards
      #bodies = np.argwhere(envs.boards == envs.BODY)
      #lengths = get_length(bodies, envs.n_boards)
      # Find indices where the value is 0.5
      #indices = np.where(np.array(rewards) == .5)
      # Modify the values at those indices
      #np.array(rewards)[indices] = .5 * lengths[indices]

      if step == 1:
         action_probs = a2c_net.actor(states)
         actions_one_hot = tf.one_hot(actions[:, 0], depth=action_probs.shape[-1])
         gathered_old_probs = tf.reduce_sum(action_probs * actions_one_hot, axis=1)

      # Learning
      loss, gathered_old_probs = learn(a2c_net, optimizer_actor, optimizer_critic, gamma, states, next_states, actions, rewards, gathered_old_probs)
      # print(loss)
...
# Compile the model
a2c_net_train.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=1e-4),
                 loss=[tf.keras.losses.CategoricalCrossentropy(), tf.keras.losses.MeanSquaredError()],
                 metrics=[['accuracy'], ['accuracy']])
...
# %%
a2c_net_train.save_weights('weights_ppo.weights.h5')
...
# %%
rewards_ep = np.mean(np.reshape(rewards_tr, (100, 1500)), axis=1)
n_fruit_eaten_ep = np.mean(np.reshape(n_fruit_eaten_tr, (100, 1500)), axis=1)
n_wall_hit_ep = np.mean(np.reshape(n_wall_hit_tr, (100, 1500)), axis=1)
n_body_hit_ep = np.mean(np.reshape(n_body_hit_tr, (100, 1500)), axis=1)
...
# %%
# Save the data
#np.savez("data_ppo.npz", max_steps=max_steps, n_fruit_eaten_ep=n_fruit_eaten_ep, n_wall_hit_ep=n_wall_hit_ep, n_body_hit_ep=n_body_hit_ep, rewards_ep=rewards_ep)